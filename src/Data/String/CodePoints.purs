...
uncons s = case _codePointAt (\_ _ -> Nothing) Just Nothing (CodePoint <<< fromEnum <<< Unsafe.charAt 0) 0 s of
  Nothing -> Nothing
  Just cp -> Just { head: cp, tail: CU.drop (CU.length (_take (\_ _ -> "") 1 s)) s }
